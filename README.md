...
- `src/mcp-server.ts` — an MCP server exposing the agent's record to Claude
  ...
  subject over a shared store handle and a pure rendering layer. See "Reading
  the record from Claude Desktop" below. `src/mcp-env.ts` is its `.env` loader
  and stdout guard, split out because the import order is load-bearing.
